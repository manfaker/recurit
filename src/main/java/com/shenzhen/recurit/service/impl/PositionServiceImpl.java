...
public class PositionServiceImpl implements PositionService {
    ...
    public PageInfo<PositionPojo> getByCompanyCode(String companyCode, String userCode, int pageNum, int pageSize) {
        ...
        if (EmptyUtils.isNotEmpty(listPosition) && !listPosition.isEmpty()) {
            List<Integer> positionIds = listPosition.stream().map(PositionPojo::getId).collect(Collectors.toList());
            Map<Integer, Integer> positionCountMap = positionUserRelationService.getCountByPosition(positionIds).stream()
                    .collect(Collectors.toMap(PositionUserRelationVO::getPositionId, PositionUserRelationVO::getPositionCount, (k1, k2) -> k1));
            listPosition.forEach(position -> {
                setInfoToPosition(position);
                Integer pisitionCount = positionCountMap.get(position.getId());
                position.setPisitionCount(EmptyUtils.isNotEmpty(pisitionCount) ? pisitionCount : NumberEnum.ZERO.getValue());
            });
        }
